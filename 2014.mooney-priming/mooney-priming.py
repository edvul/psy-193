...
demofile = open(os.path.join('subjectsdemo.txt'), 'a')
demofile.write(str(subject))
if(not DEBUG):
	FILE.write(', %s' 		% raw_input('Enter Name' ).strip())
	FILE.write(', %s' 		% raw_input('Enter Age' ).strip())
	FILE.write(', %s\n' 	% raw_input('Enter Gender').strip())
demofile.close()

def readList(filename):
	listfile = open(os.path.join(stimPath, filename), 'r')
	return([x for x in listfile.readlines().strip()])
...
showText(SCREEN, 'Part 1: In the following portion of the study, you will be presented a list of 25 words. Please study them in the time allotted. You will have 40 seconds to view the word list. Try to remember as many of them as you can.  Your performance in this task will increase your ability to parse the ambiguous images presented in Part 2. Please press <Enter > to move on to the list of words.')
waitForKey(K_SPACE)
